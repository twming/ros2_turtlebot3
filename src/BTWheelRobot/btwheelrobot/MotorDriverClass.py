...
def setMotor(S1,S2,S3,S4,timeInMs):
    GPIO.output(gPin.M1,S1)
    GPIO.output(gPin.M2,S2)
    GPIO.output(gPin.M3,S3)
    GPIO.output(gPin.M4,S4)
    time.sleep(timeInMs)
...
class MotorDriverClass(Node):
    ...
    def listener_callback(self, msg):
        ...
        if (GPIO.input(gPin.BUT)):
            GPIO.output(gPin.LED,GPIO.HIGH)
